        ...
        <div className="max-w-container-max mx-auto px-margin-desktop grid grid-cols-1 lg:grid-cols-12 gap-gutter relative z-10">
          <div className="lg:col-span-5 space-y-8 self-center">
            <div className="inline-flex items-center gap-3 py-2 px-4 border border-marketplace-bronze/30 bg-marketplace-bronze/5">
              <span className="material-symbols-outlined text-marketplace-bronze" style={{ fontVariationSettings: "'FILL' 1" }}>
                verified
              </span>
            ...
            <div className="grid grid-cols-2 gap-8 border-y border-black/10 py-8">
              <div>
                <p className="font-label-sm text-label-sm uppercase text-text-muted mb-1">Experience</p>
                <p className="font-headline-sm text-headline-sm">{stitcher.experience ?? "Master Level"}</p>
              </div>
              <div>
                <p className="font-label-sm text-label-sm uppercase text-text-muted mb-1">Expertise</p>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  ...
                  className="bg-primary text-white font-label-md text-label-md uppercase py-5 px-10 tracking-[0.2em] hover:bg-marketplace-bronze transition-all duration-500 text-center"
                >
                  Start Stitching Project
                </Link>
                <Link
                  href="/contact"
                  className="border border-black font-label-md text-label-md uppercase py-5 px-10 tracking-[0.2em] hover:bg-black hover:text-white transition-all duration-500 text-center"
                >
                  Message Artisan
                </Link>
              </div>
            ...
            <div className="relative w-full aspect-[4/5] max-w-[500px]">
              ...
              {stitcher.location && (
                <div className="absolute bottom-12 -left-12 bg-white p-8 border border-black max-w-xs hidden md:block">
                  ...
                  <p className="font-headline-sm text-headline-sm leading-tight">{stitcher.location}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <>
            ...
            <div className="max-w-container-max mx-auto px-margin-desktop">
              <div className="flex justify-between items-end mb-16">
                <div>
                  ...
                  <h2 className="font-display-lg text-display-lg leading-none">The Artisan&apos;s Archive</h2>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
                {stitcher.portfolio!.map((item) => (
                  <div key={item.title} className={`${spanClass[item.span]} group cursor-pointer overflow-hidden`}>
                    <div className={`relative overflow-hidden mb-6 ${item.span === "wide" || item.span === "square" ? "" : "aspect-[4/5]"}`}>
                      <div
                      ...
                      <div className="absolute bottom-0 left-0 w-full bg-black/80 text-white p-6 translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                        <p className="font-label-sm text-label-sm uppercase tracking-widest">{item.subtitle}</p>
                      </div>
                    </div>
          ...
          <section className="py-24 bg-surface-container-low border-y border-black/5">
            <div className="max-w-container-max mx-auto px-margin-desktop grid grid-cols-1 lg:grid-cols-2 gap-24">
              ...
              <Reveal delay={150} className="relative bg-white border border-black p-12 lg:p-20 self-start">
                <h4 className="font-headline-md text-headline-md mb-8">Client Testimonials</h4>
                <div className="space-y-12">
                  {stitcher.testimonials!.map((t, i) => (
                    <blockquote key={t.author} className={i > 0 ? "relative pt-12 border-t border-black/5" : "relative"}>
                      <p className="font-body-lg italic text-on-surface mb-4">&ldquo;{t.quote}&rdquo;</p>
      ...
      <section className="py-32 bg-primary text-white text-center relative overflow-hidden">
        <div className="max-w-2xl mx-auto px-margin-mobile relative z-10">
          ...
          <p className="text-body-lg font-body-lg opacity-80 mb-12">
            Limited monthly slots available for custom bridal and formal
            projects. Secure your consultation today.
          </p>
          <Link
            href="/tailoring/configure"
